...
public:
	...
	ImageFeatures(Mat image, int code) {
		cvtColor(image, this->image, CV_RGB2GRAY);
		Ptr<SURF> detector = SURF::create(minHessian);
...
int main(int argc, char** argv)
{
	...
	int start_s = clock();
	// For Single Image
	img_scene = imread(basePath2 + "img2.jpg");
	scene = ImageFeatures(img_scene, CV_RGB2GRAY);
	Mat view_scene;
	resize(borderedObject(img_scene, surfedImage(object, scene)), view_scene, Size(640, 480));
	imshow("WOW", view_scene);
	int stop_s = clock();
